...
pub async fn create_game(pool: &MySqlPool, white_player_id: i32, black_player_id: i32) -> Result<i32, sqlx::Error> {
...
pub async fn update_users_statistics(pool: &MySqlPool, white_player_id: i32, black_player_id: i32, result: &GameStatus) -> Result<(), sqlx::Error> {
    ...
    match result {
        GameStatus::WhiteWin => {
            sqlx::query!(
                "UPDATE statistics SET games_won = games_won + 1, last_game_at = ? WHERE user_id = ?",
                Utc::now(),
                white_player_id
            )
            .execute(pool)
            ...
            sqlx::query!(
                "UPDATE statistics SET games_lost = games_lost + 1, last_game_at = ? WHERE user_id = ?",
                Utc::now(),
                black_player_id
            )
            .execute(pool)
        ...
        GameStatus::Draw => {
            sqlx::query!(
                "UPDATE statistics SET games_drawn = games_drawn + 1, last_game_at = ? WHERE user_id = ?",
                Utc::now(),
                white_player_id
            )
            .execute(pool)
